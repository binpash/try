...
ptx_syntax_spec: list[SyntaxSpecification] = [
    [
        [
            ...
            Flag("-t", alias=["--typeset-mode"]),  # Not implemented, but included as per man page
            Integer(flag="-w", alias=["--width"]),
            Flag("--help"),
            Flag("--version"),
        ],
        [Path(arity=Arity.ZERO_OR_MORE)],  # INPUT can be zero or more
        [Path(arity=Arity.ZERO_OR_ONE)]    # OUTPUT is optional and should be one or none
    ]
]
