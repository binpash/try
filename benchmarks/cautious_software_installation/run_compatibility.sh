...
for case in $(find . -maxdepth 1 -mindepth 1 -type d); do
  ...
  run_benchmark() {
    ...
    hash_run_script=$(docker run --privileged -it --rm --name try-prepostinstall -v /tmp:/tmp try_prepostinstall_benchmarks /bin/bash "$run_script" --hash)
    hash_try_run_script=$(docker run --privileged -it --rm --name try-prepostinstall -v /tmp:/tmp try_prepostinstall_benchmarks /bin/bash "$try_run_script" --hash)
