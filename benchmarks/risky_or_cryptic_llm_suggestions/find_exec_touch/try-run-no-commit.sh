...
if $time_only; then
  time_output=$(/usr/bin/time -f "%e" try -n bash script.sh 2>&1 | grep -E '^[0-9]+\.[0-9]+$' | head -1)
  echo "$time_output"
elif $print_time; then
  /usr/bin/time -f "%e" try -n bash script.sh
else
  /usr/bin/time -f "%e" try -n bash script.sh &> /dev/null
fi
