...
cd $prefix/find_exec_touch

# Run script.sh and handle time output based on the flags
if $time_only; then
  time_output=$(/usr/bin/time -f "%e" try -y -t x.log bash script.sh 2>&1 >/dev/null)
  echo "$time_output"
elif $print_time; then
  /usr/bin/time -f "%e" try -y -t x.log bash script.sh
else
  /usr/bin/time -f "%e" try -y -t x.log bash script.sh &> /dev/null
fi
