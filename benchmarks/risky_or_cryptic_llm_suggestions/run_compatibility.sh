...
OUTPUT_DIR="${BASE_DIR}/../results/llm_scripts"
...
for case_name in $BENCHMARK_CASES; do
  ...
  run_benchmark() {
    ...
    hash_run_script=$(docker run --privileged --rm --name try-llm -v /tmp:/tmp try_llm_benchmarks /bin/bash "$run_script" --hash)
    hash_try_run_script=$(docker run --privileged --rm --name try-llm -v /tmp:/tmp try_llm_benchmarks /bin/bash "$try_run_script" --hash)
